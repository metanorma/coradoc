...
#
# This file contains HTML5 entity mappings (2137 lines).
# The length is intentional - it is a complete mapping of all HTML entities.
#
# Regeneration command:
#   curl -LO https://html.spec.whatwg.org/entities.json
#   ruby -e 'require "json"; puts JSON.load_file("entities.json").map { |k, v| \
#     next unless k.end_with? ";"; \
#     "\\"#{k.slice(1, k.length - 2)}\\" => \\"#{v["codepoints"].map { |c| "\\\\u{%x}" % c }.join "" }\\"" \
#   }.compact.join ",\\n"'
...
module Coradoc
  module Markdown
    module Parser
      # curl -LO https://html.spec.whatwg.org/entities.json
      # ruby -e 'require "json"; puts JSON.load_file("entities.json").map { |k, v| next unless k.end_with? ";"; "\\"#{k.slice(1, k.length - 2)}\\" => \\"#{v["codepoints"].map { |c| "\\\\u{%x}" % c }.join "" }\\"" }.compact.join ",\\n"'
      HTML_ENTITIES = {
        'AElig' => "\u{c6}",
        'AMP' => "\u{26}",
        'Aacute' => "\u{c1}",
