...
RSpec.describe "Coradoc::Parser::Asciidoc::Citatione" do
  describe ".parse" do
    it "parses multiple types of citation" do
      ...
      expect(ast).to eq([{ citation: { comment: [{ text: "some reference",
                                                   line_break: "\n" }] } }])
      ...
      expect(ast).to eq([{ citation: { cross_reference: [
                          { href_arg: "xref_anchor" }, { text: "display text" }
                        ] } }])

      ast = parser.parse("[.source]\n<<xref_anchor,section=1>>\n")
      expect(ast).to eq([{ citation: { cross_reference: [
                          { href_arg: "xref_anchor" }, { key: "section", delimiter: "=",
                                                         value: "1" }
                        ] } }])

      ast = parser.parse("[.source]\n<<xref_anchor>>some reference\n")
      obj = [{ citation: { cross_reference: [{ href_arg: "xref_anchor" }],
                           comment: [{ text: "some reference",
                                       line_break: "\n" }] } }]
