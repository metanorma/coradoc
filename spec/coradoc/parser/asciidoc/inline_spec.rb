...
RSpec.describe "Coradoc::Parser::Asciidoc::Inline" do
  describe ".parse" do
    it "parses various inline text formattings" do
      ...
      obj = [{:span_constrained=>
               {:attribute_list=>
                 {:attribute_array=>[{:positional=>"small"}]},
                :text=>"span"}}]
      expect(ast).to eq(obj)

      ast = parser.parse('[small]##span2##')
      obj = [{:span_unconstrained=>
               {:attribute_list=>
                 {:attribute_array=>[{:positional=>"small"}]},
                :text=>"span2"}}]
      expect(ast).to eq(obj)
      ...
      ast = parser.parse("footnote:[some text *with bold*]")
    ...
    it "parses cross-references" do
      ...
      expect(ast).to eq([{:cross_reference=>[{:href_arg=>"xref_anchor"}, {:text=>"display text"}]}])


      ast = parser.parse("<<xref_anchor,section=1>>")
      expect(ast).to eq([{:cross_reference=>[{:href_arg=>"xref_anchor"},
                          {:key=>"section", :delimiter=>"=", :value=>"1"}]}])
