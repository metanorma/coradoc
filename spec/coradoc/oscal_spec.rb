...
RSpec.describe Coradoc::Oscal do
  describe ".parse" do
    it "parses the asciidoc to standard doc", skip: true do
      sample_file = Coradoc.root_path.join(
        "spec", "fixtures", "sample-oscal.adoc"
      )

      document = Coradoc::Oscal.parse(sample_file)
      ...
      control_section = section[:sections].first
      ...
      purpose_section = section[:sections][1]
      ...
      highlight_5_9 = section[:sections][33][:contents][2][:highlight]
      expect(highlight_5_9[:id]).to eq("scls_5-9")
      expect(highlight_5_9[:text]).to eq("Inventory")
    end

    it "parses the new sample doc as well" do
      sample_file = Coradoc.root_path.join(
        "spec", "fixtures", "new-sample-oscal.adoc"
      )

      document_in_yaml = Coradoc::Oscal.parse_to_yaml(sample_file)

      # File.open("./oscal.yml", "w") do |file|
      #   file.write(document_in_yaml)
      # end
    end
  end
end
