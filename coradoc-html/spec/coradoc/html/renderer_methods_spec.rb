...
RSpec.describe Coradoc::Html::Renderer do
  ...
  describe '#extract_title' do
    it 'returns the title from a StructuralElement' do
      doc = CoreModel::DocumentElement.new(title: 'My Document', children: [])
      result = renderer.send(:extract_title, doc)
      expect(result).to eq('My Document')
    end

    it 'returns nil for non-StructuralElement objects' do
      block = CoreModel::Block.new(title: 'Block Title')
      result = renderer.send(:extract_title, block)
      expect(result).to be_nil
    end

    it 'returns nil when title is nil' do
      doc = CoreModel::DocumentElement.new(title: nil, children: [])
      result = renderer.send(:extract_title, doc)
      expect(result).to be_nil
    end
  end

  describe '#build_toc' do
    it 'returns a TocBuilder with defaults when options are empty' do
      toc_builder = renderer.send(:build_toc, nil, {})
      expect(toc_builder).to be_a(Coradoc::Html::TocBuilder)
    end

    it 'passes sectnums option to TocBuilder' do
      toc_builder = renderer.send(:build_toc, nil, { sectnums: true })
      expect(toc_builder).to be_a(Coradoc::Html::TocBuilder)
    end

    it 'passes toclevels and sectnumlevels options to TocBuilder' do
      toc_builder = renderer.send(:build_toc, nil, {
                                    toclevels: 3,
                                    sectnumlevels: 2,
                                    sectnums: true
      ...
      doc = CoreModel::DocumentElement.new(
        title: 'Doc',
        children: [
          CoreModel::SectionElement.new(id: 's1', title: 'Section 1', level: 1, children: [])
        ]
      )
      toc = toc_builder.build(doc)
      expect(toc).to be_a(CoreModel::Toc)
    end
  end
end
